...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 22 16:18:20 2023
...
from dtmm2.conf import NCDTYPE,NFDTYPE, FDTYPE, CDTYPE, NUMBA_PARALLEL, NUMBA_TARGET, NUMBA_CACHE, BETAMAX , DTMMConfig, get_default_config_option
from dtmm2.wave import planewave, betaphi, wave2eigenwave
from dtmm2.window import aperture
from dtmm2.fft import fft2, ifft2, fft, ifft
from dtmm2.wave import betaxy, eigenmask, eigenmask1
from dtmm2.window import blackman
from dtmm2.tmm import alphaf ,fvec2E, E2fvec
from dtmm2.tmm import fvec as field4
from dtmm2.data import refind2eps
from dtmm2.jones import jonesvec
...
def modes2ffield(mask, modes, out = None):
    ...
    shape = mask.shape[-2:]
    if mask.ndim == 2:
        shape = modes.shape[:-2] + modes.shape[-1:] + shape
        if out is None:
        ...
        shape = modes[0].shape[:-2] + (len(mask),) + modes.shape[-1:] + shape
        if out is None:
            out = np.zeros(shape =shape, dtype = CDTYPE )
        else:
            out[...] = 0.
