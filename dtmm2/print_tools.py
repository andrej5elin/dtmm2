...
def print_layer_rate(n_frames, t0, t1 = None, message = "... processed", level = None):
    ...
    if dtmm2.conf.DTMMConfig.verbose >= 2:
        if t1 is None:
...
def print1(*args, **kwargs):
    ...
    if dtmm2.conf.DTMMConfig.verbose >= 1:
        print(*args,**kwargs)
...
def print_frame_rate(n_frames, t0, t1 = None, message = "... processed"):
    ...
    if dtmm2.conf.CDDMConfig.verbose >= 2:
        if t1 is None:
            t1 = time.time()#take current time
...
def disable_prints():
    ...
    value = dtmm2.conf.CDDMConfig.verbose
    dtmm2.conf.CDDMConfig.verbose = - abs(value)
    return value
    
def enable_prints(level = None):
    """Enable message printing. Returns previous verbosity level"""
    #set verbosity to positve value
    value = dtmm2.conf.CDDMConfig.verbose if level is None else int(level)
    dtmm2.conf.CDDMConfig.verbose =  abs(dtmm2.conf.CDDMConfig.verbose)
    return value
