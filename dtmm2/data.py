...
from dtmm2.conf import FDTYPE, CDTYPE, NFDTYPE, NCDTYPE, NUMBA_CACHE,\
NF32DTYPE,NF64DTYPE,NC128DTYPE,NC64DTYPE, DTMMConfig, deprecation
from dtmm2.rotation import rotation_matrix_x,rotation_matrix_y,rotation_matrix_z, rotate_vector, rotation_angles, rotation_matrix, rotate_diagonal_tensor
from dtmm2.wave import betaphi, k0
import dtmm2.fft #import fft2, ifft2#, #fft, ifft
from dtmm2.linalg import tensor_eig
...
matrix2eps = matrix2tensor
eps2matrix = tensor2matrix

def permutations(*indices):
    """Returns all possible second rank or third rank tensor indices permutations"""
    if len(indices) == 2:
        i,j = indices
        if i == j:
            return ((i,i),)
        ...
        if i ==j:
            ...
            return ((i,j,i),(j,i,i),(i,i,j))
        elif j == k:
            return ((i,j,j),(j,i,j),(j,j,i))
        return ((i,j,k),(j,k,i),(k,i,j),(j,i,k),(i,k,j),(k,j,i))
    else:
        raise ValueError('2 or 3 arguments required')
    
def compress_tensor_indices(*indices):
    """Converts standard third or second rank tensor indices into a compressed notation"""
    # second rank tensor case
    if len(indices) == 2:
        i,j = indices
        if not i in (1,2,3):
            raise ValueError(f'Invalid index i {i}')    
        if not j in (1,2,3):
            raise ValueError(f'Invalid index j {j}')    
        if i == j:
            return i
        ...
        if not j in (1,2,3):
            raise ValueError(f'Invalid index j {j}')
        if not k in (1,2,3):
            raise ValueError(f'Invalid index k {k}')
        if j == k:
            return i,j
        if j == 2 and k == 3 or j ==3 and k ==2:
            return i, 4
        if j == 1 and k == 3 or j ==3 and k ==1:
            return i, 5
...
def expand_tensor_indices(*indices):
    """Converts compressed notation of thord or second rank tensor into standard form"""
    if len(indices) == 1:
        i, = indices
        if not i in (1,2,3):
            raise ValueError(f'Invalid index i {i}')
        ...
        raise ValueError('1 or 2 arguments required')

        
def chi2array(**d):
    """Constructs chi2 tensor from input keyword arguments.
    
    >>> c = chi2array(d33 = 4)
    
    """
    out = np.zeros(shape = (3,6), dtype = FDTYPE)

    coeff = {}

    for key, value in d.items():
        if len(key) == 3 and key[0] == 'd':
            i,j = int(key[1]), int(key[2])
            i,j,k = expand_tensor_indices(i,j)
            cs = set([compress_tensor_indices(*c) for c in permutations(i,j,k)])
            for key in cs:
                default = coeff.get(key)
                if default is None:
                    coeff[key] = value
                elif default != value:
                    raise ValueError(f'Chi2 symmetry violation found in coefficient ({i}, {j})')
  
        else:
             raise ValueError(f'Invalid coefficent name {key}')
