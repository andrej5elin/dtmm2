...
from dtmm2.conf import NCDTYPE, NFDTYPE, NF32DTYPE, NF64DTYPE, NC64DTYPE,NC128DTYPE, \
...
@jit([(NF32DTYPE[:, :], NF32DTYPE[:], NF32DTYPE[:]),
      ...
      ], nopython=True, cache=NUMBA_CACHE, fastmath=NUMBA_FASTMATH)
...
@nb.guvectorize([(NF32DTYPE[:, :], NC64DTYPE[:], NC64DTYPE[:]),
                 (NF64DTYPE[:, :], NF64DTYPE[:], NFDTYPE[:]), ], "(n,n),(n)->(n)", cache=NUMBA_CACHE,
                fastmath=NUMBA_FASTMATH, target=NUMBA_TARGET)
def rotate_complex_vector(rotation_matrix, vector, out):
    """
    Rotates vector <vector> using rotation matrix <rotation_matrix>
    rotate_vector(R, vector)
